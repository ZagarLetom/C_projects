...
"-b -e -n -s -t test_files/test_1_cat.txt"
"-b -e -n -s -t test_files/test.txt"
...
GNU_FLAGS="--number-number-nonblank-squeeze-blankt"
./s21_cat "$GNU_FLAGS" test_files/test.txt > buff.txt
cat -tbns test_files/test.txt > buff_2.txt
DIFF="$(diff -s buff.txt buff_2.txt)"
let "COUNTER++"
    if [ "$DIFF" == "Files buff.txt and buff_2.txt are identical" ]
    then
        let "SUCCESS++"
        echo "$COUNTER - Success "$GNU_FLAGS" test_files/test.txt"
    else
        let "FAIL++"
        echo "$COUNTER - Fail "$GNU_FLAGS" test_files/test.txt"
    fi
rm -rf buff.txt buff_2.txt
GNU_FLAGS="--number-number-nonblank-squeeze-blanke"
./s21_cat "$GNU_FLAGS" test_files/test.txt > buff.txt
cat -ebns test_files/test.txt > buff_2.txt
DIFF="$(diff -s buff.txt buff_2.txt)"
let "COUNTER++"
    if [ "$DIFF" == "Files buff.txt and buff_2.txt are identical" ]
    then
